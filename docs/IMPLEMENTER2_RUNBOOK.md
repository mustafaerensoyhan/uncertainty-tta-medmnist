...
# 2-5. All four figures at 300 DPI
...
- `figures/fig2_ece.pdf` — 6 dataset groups, bars per strategy (baseline,
  entropy, maxprob, variance, ts_entropy; `top5` only if present in
  `seed_stability.csv`), error bars = 3-seed std, dashed no-TTA line per group.
...
- **Bootstrap CSV:** only **dermamnist** prediction arrays exist (as
  ...
  machine.)
- **Fig 2 / Fig 5:** fully reproduce from `seed_stability.csv`. The Fig 5 plan
  values are embedded as `PLAN_FIG5` for cross-checking — they match
  `seed_stability.csv` exactly. `top5` is absent from `seed_stability.csv`, so
  Fig 2 omits it. EfficientNet-B0 overlay covers 5 datasets (no organamnist in
  `effb0_seed_stability.csv`).
- **Fig 4:** `dermamnist_ablation_aug.csv` (resnet18) is missing, so the full
  6×10 matrix is incomplete. The default run fails clearly and lists the missing
  dataset; `--allow-partial` renders the 5 available rows with the Derma row left
  blank (no numbers invented).
- **Fig 3:** PathMNIST/BloodMNIST prediction arrays are not on disk, so this
  figure cannot be built here yet. The script prints the exact missing files and
  the `run_weighted_tta` commands; once those arrays exist the 2×2 renders.
...
# DermaMNIST augmentation ablation (writes results/dermamnist_ablation_aug.csv):
python -m scripts.ablate_augmentations --dataset dermamnist
```

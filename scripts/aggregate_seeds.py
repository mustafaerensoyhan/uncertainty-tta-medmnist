...
def main() -> int:
    ...
    for ds in args.datasets:
        seed_files = sorted(f for f in rdir.glob(f"{ds}_seed*_weighted_tta.csv")
                            if _SEED_RE.search(f.name))
        if len(seed_files) < 2:
            if seed_files:
                print(f"[skip] {ds}: only {len(seed_files)} seed file(s) — need >=2 for std.")
            continue
        ...
        for strat, g in df.groupby("strategy", sort=False):
            row = {"dataset": ds, "strategy": strat, "n_seeds": n_seeds}
            for m in _METRICS:
                if m in g:
                    row[f"{m}_mean"] = round(float(g[m].mean()), 4)
                    row[f"{m}_std"] = round(float(g[m].std(ddof=1)), 4)
            all_rows.append(row)
        print(f"[ok]   {ds}: aggregated {n_seeds} seeds "
              f"({', '.join(f.name.split('_')[1] for f in seed_files)}).")

    if not all_rows:
        print("\nNo multi-seed result files found. Run train_baseline + run_weighted_tta "
